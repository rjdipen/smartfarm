    ...
    <div id="myModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{action('Store\SsubcatController@save')}}" method="post" enctype="multipart/form-data">
                    {{csrf_field()}}
                    <div class="modal-body">
                        <div class="input-group">
                            <span class="input-group-addon">Category</span>
                            <select class="form-control m-input" name="scatID" >
                                <option value="">Select Category </option>
                                @foreach($scat as $row)
                                    <option value="{{$row->scatID}}"><i class="{{$row->icon}}"></i> <span>{{$row->name}}</span></option>
                                @endforeach
                            </select>
                        </div><br>
                        ...
                        <div class="input-group">
                            <span class="input-group-addon">SubCategory Icon</span>
    ...
    <div id="ediModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-teal-400">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h5 class="modal-title"><i class="icon-pencil7"></i> Edit SubCategory</h5>
                </div>

                <form action="{{action('Store\SsubcatController@edit')}}" method="post" enctype="multipart/form-data">
                    {{csrf_field()}}
