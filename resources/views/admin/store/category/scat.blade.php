...
@extends('admin.box.store.scatBox')
@section('title')
    Category
@endsection
@section('content')
                            ...
                            <tr>
                                ...
                                <td class="text-right">
                                    <button class="btn btn-xs btn-success no-padding mr-5 ediBtn"
                                            data-name="{{$row->name}}"
                                            data-icon="{{$row->icon}}"
                                            data-id="{{$row->scatID}}"  data-toggle="modal" data-target="#ediModal" title="Edit"><i class="icon-pencil5"></i></button>
                                    <a class="btn btn-xs btn-danger no-padding" href="{{action('Store\ScatController@del', ['id' => $row->scatID])}}"  onclick="return confirm('Are you sure to delete?')" title="Delete"><i class="icon-bin"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
